<?php namespace App\Http\Controllers;
	...
	class AdminQuiosqueController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Com café","name"=>"((case when inscricoes.refeicao = 'QUIOSQUE_COM_CAFE' then 1 else 0 end) + (select count(*) from inscricoes ins where ins.numero_inscricao_responsavel = inscricoes.numero and ins.refeicao = 'QUIOSQUE_COM_CAFE' and ins.presencaConfirmada = 1)) as comcafe_total","callback"=>function($row) {
				if ($row->comcafe_total == 0)
					return '';
				return '<span class="badge bg-red">'. $row->comcafe_total .'</span>';
			}];
			$this->col[] = ["label"=>"Sem café","name"=>"((case when inscricoes.refeicao = 'QUIOSQUE_SEM_CAFE' then 1 else 0 end) + (select count(*) from inscricoes ins where ins.numero_inscricao_responsavel = inscricoes.numero and ins.refeicao = 'QUIOSQUE_SEM_CAFE' and ins.presencaConfirmada = 1)) as semcafe_total","callback"=>function($row) {
